...
@given(
    parsers.parse("A package containing 2 objects of {source_package_type}"),
...
@given(
    parsers.parse("We have another path to save the objects to {new_package_type}"),
    target_fixture="new_path_to_save_to",
)
...
@when(
    parsers.parse(
        "we transform all the packages found into the new {new_package_type} package format"
    )
)
...
@then(
    parsers.parse(
        "the newly transformed package should contain the same files but in the "
        "{new_package_type} format"
    )
)
...
@scenario(
    "package_transform.feature",
    "Capture One session with a .DS_Store file transform to a HathiTrust Tiff package",
)
...
@given(
    "A Capture One session package containing 2 objects and a .DS_Store file in it",
    target_fixture="capture_one_session",
)
...
@scenario(
    "package_transform.feature",
    "Capture One session with a thumbs.db file transform to a HathiTrust Tiff package",
)
...
@given(
    "A Capture One session package containing 2 objects and a thumbs.db file in it",
    target_fixture="capture_one_session",
)
