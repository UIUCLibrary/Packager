...
def testPkg(args = [:]){
    ...
    def failure = {}
    if (args['post']){
        cleanup =  args['post']['cleanup'] ? args['post']['cleanup']: {}
        successful = args['post']['success'] ? args['post']['success']: {}
        failure = args['post']['failure'] ? args['post']['failure']: {}
    }
...
def buildPkg(args = [:]){
    ...
    if(args['post']){
        def post = args['post']
        cleanup =  post['cleanup'] ? post['cleanup']: {}
        successful = post['success'] ? post['success']: {}
        failure = post['failure'] ? post['failure']: {}
    }
