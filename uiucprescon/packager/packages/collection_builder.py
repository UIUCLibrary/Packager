...
def _build_ds_instance(item, name, path):
    new_instantiation = Instantiation(category=InstantiationTypes.ACCESS, parent=item)
...
def build_ds_batch(root):
    ...
    new_batch.component_metadata[Metadata.PACKAGE_TYPE] = PackageTypes.DS_HATHI_TRUST_SUBMISSION
...
def build_bb_batch(root) -> Package:
    ...
    for directory in filter(lambda i: i.is_dir(), os.scandir(root)):
        ...
        new_object.component_metadata[Metadata.PACKAGE_TYPE] = PackageTypes.BRITTLE_BOOKS_HATHI_TRUST_SUBMISSION
        build_bb_package(new_object, path=directory.path)
    return new_batch


def build_capture_one_instance(new_item, name, path):
    new_instantiation = Instantiation(category=InstantiationTypes.PRESERVATION, parent=new_item)
...
def build_capture_one_batch(root) -> Package:
    ...
    for object_name in sorted(group_ids):
        ...
        new_object.component_metadata[Metadata.PACKAGE_TYPE] = PackageTypes.CAPTURE_ONE_SESSION
...
def build_digital_library_compound_item(new_item, path, item_name):
    ...
    access_instance = Instantiation(category=InstantiationTypes.ACCESS, parent=new_item)
    access_instance.files.append(access_file)

    preservation_instance = Instantiation(category=InstantiationTypes.PRESERVATION, parent=new_item)
...
def build_digital_library_compound_batch(root):
    ...
    for dir_ in filter(lambda i: i.is_dir(), os.scandir(root)):
        ...
        new_object.component_metadata[Metadata.ID] = dir_.name
        new_object.metadata[Metadata.PATH] = dir_.path
        new_object.component_metadata[Metadata.PACKAGE_TYPE] = PackageTypes.DIGITAL_LIBRARY_COMPOUND
